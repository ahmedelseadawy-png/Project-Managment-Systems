-- V140_17 Align actual progress with approved physical site progress.
...
with latest_site as (
  ...
  select
    contract_item_id,
    min(progress_date) filter (where coalesce(progress_percent, 0) > 0) as actual_start_date,
    min(progress_date) filter (where coalesce(progress_percent, 0) >= 100) as actual_finish_date
  from public.site_progress_updates
  where lower(coalesce(status, '')) = 'approved'
  group by contract_item_id
), item_locations_raw as (
  select
    ps.*,
    ...
    ci.villa_id,
    ci.building_id,
    coalesce(ls.progress_percent, 0)::numeric(9,3) as approved_site_progress_percent,
    sd.actual_start_date,
    sd.actual_finish_date,
    coalesce(
      ci.villa_id::text,
      ci.building_id::text,
  ...
  left join latest_site ls on ls.contract_item_id = ps.contract_item_id
  left join site_dates sd on sd.contract_item_id = ps.contract_item_id
  ...
  select
    ...
    case
      when sum(coalesce(contract_value, 0)) > 0 then
        sum(coalesce(approved_site_progress_percent, 0) * coalesce(contract_value, 0))
        / nullif(sum(coalesce(contract_value, 0)), 0)
      else avg(coalesce(approved_site_progress_percent, 0))
  ...
  select
    *,
    ...
    trim(both ' | ' from concat_ws(' | ',
      case
        when certified_progress_percent > 0 and coalesce(site_progress_percent, 0) = 0
          then 'Certified progress exists but no approved site progress.'
      end,
      case
        when coalesce(site_progress_percent, 0) > certified_progress_percent
          then 'Executed work not fully certified.'
      end
    )) as progress_warning
  from progress
)
select
  ...
  subcontractor_code,
  subcontractor_name,
  nullif(progress_warning, '') as progress_warning
from health;
...
with item_locations_raw as (
  ...
  select
    ...
    count(*) filter (where progress_warning is not null) as warning_count,
    trim(both ' | ' from concat_ws(' | ',
      case when count(*) filter (where progress_warning like '%Certified progress exists but no approved site progress.%') > 0
        then 'Certified progress exists but no approved site progress.' end,
      case when count(*) filter (where progress_warning like '%Executed work not fully certified.%') > 0
        then 'Executed work not fully certified.' end
    )) as progress_warning
...
select
  ...
  coalesce(bc.warning_count, 0)::int as warning_count,
  nullif(bc.progress_warning, '') as progress_warning
